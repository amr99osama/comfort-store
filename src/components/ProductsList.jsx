    ...
    return (
        <div className='mt-12 grid gap-y-4'>
            {products.map((product) => {
                ...
                const { id } = product;
                return (
                    <Link key={id} to={`/products/${id}`} className='p-8 rounder-lg flex flex-col sm:flex-row gap-y-4 flex-wrap bg-base-100 shadow-lg hover:shadow-2xl transition-shadow duration-300 group'>
